...
public class CilVideoJoypadTests
{
    ...
    {
        var link = new LinkerType().Link([new Koh.Linker.LinkerInput("cil", model)]);
    ...
    /// <c>Koh.Emulator.Ppu.Ppu</c>) — one full vblank wait (~17.5K M-cycles) on top of
    ...
    //
    // The emulator poisons VRAM/OAM with 0xFF at power-on (Koh.Emulator.Bus.Mmu ctor), so reading
