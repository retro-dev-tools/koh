...
/// (<c>Koh.Emulator.Dma.OamDma</c>), landing 160 real bytes at 0xFE00. Deliberately keeps its own
...
public class CilSpritePaletteHardwareTests
{
    ...
    /// <c>Koh.Emulator.Bus.Mmu.ReadByte</c>'s bus-lock comment), so any ROM code fetched during
    ...
    /// <see cref="Koh.Emulator.Dma.OamDma"/> path, just without a ROM-side wait loop that
    ...
    // writing Hardware.DMA driving the real Koh.Emulator.Dma.OamDma path.
    ...
    // (Koh.Emulator.Dma.OamDma.TickT is called once per T-cycle, 4x per M-cycle — see
    // GameBoySystem.TickOneMCycle; Koh.Emulator.Tests.OamDmaTests uses the same 648/700 margin).
