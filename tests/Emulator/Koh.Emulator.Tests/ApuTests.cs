...
public class ApuTests
{
    ...
    {
        var apu = new Koh.Emulator.Apu.Apu { Enabled = false };
    ...
    {
        var apu = new Koh.Emulator.Apu.Apu { Enabled = true };
    ...
    {
        var apu = new Koh.Emulator.Apu.Apu();
        // Off by default.
    ...
    public async Task Cgb_WaveRam_Access_While_Channel_On_Always_Redirects_No_Miss()
    {
        ...
        var apu = new Koh.Emulator.Apu.Apu(HardwareMode.Cgb);
        apu.Write(0xFF26, 0x80);
        for (int i = 0; i < 16; i++)
            apu.Write((ushort)(0xFF30 + i), (byte)(i * 0x11));
        apu.Write(0xFF1A, 0x80); // DAC on
