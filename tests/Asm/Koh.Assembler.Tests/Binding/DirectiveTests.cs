...
namespace Koh.Assembler.Tests.Binding;
...
public class DirectiveTests
{
    ...
    public async Task Assert_FalseCondition_ReportsError()
    {
        ...
        await Assert
            .That(model.Diagnostics.Any(d => d.Message.Contains("Assertion failed")))
            .IsTrue();
    }
    ...
    public async Task Assert_FalseWithMessage()
    {
        ...
        await Assert.That(model.Success).IsFalse();
        await Assert
            .That(model.Diagnostics.Any(d => d.Message.Contains("value must be nonzero")))
            .IsTrue();
    }
        ...
        await Assert
            .That(
                model.Diagnostics.Any(d =>
                    ...
                    && d.Message.Contains("this is a warning")
                )
            )
            .IsTrue();
    }
    ...
    public async Task Pops_WithoutPushs_ReportsError()
    {
        ...
        await Assert.That(model.Success).IsFalse();
        await Assert
            .That(model.Diagnostics.Any(d => d.Message.Contains("POPS without matching PUSHS")))
            .IsTrue();
    }
    ...
    public async Task StaticAssert_FalseWithMessage()
    {
        ...
        await Assert.That(model.Success).IsFalse();
        await Assert
            .That(model.Diagnostics.Any(d => d.Message.Contains("static check failed")))
            .IsTrue();
    }
    ...
    public async Task StaticAssert_UnresolvableExpr_ReportsError()
    {
        ...
        await Assert
            .That(
                model.Diagnostics.Any(d =>
                    d.Message.Contains("could not be evaluated at assembly time")
                )
            )
            .IsTrue();
    }
    ...
    public async Task Assert_WarnSeverity_ProducesWarning()
    {
        ...
        await Assert
            .That(
                model.Diagnostics.Any(d =>
                    d.Severity == DiagnosticSeverity.Warning && d.Message.Contains("soft warning")
                )
            )
            .IsTrue();
    }
    ...
    public async Task Assert_NoCondition_ReportsError()
    {
        ...
        await Assert.That(model.Success).IsFalse();
        await Assert
            .That(model.Diagnostics.Any(d => d.Message.Contains("ASSERT requires a condition")))
            .IsTrue();
    }
    ...
    public async Task AbortOnMissingIncbin_MissingFile_ReportsDiagnostic()
    {
        ...
        var vfs = new Koh.Assembler.VirtualFileResolver();
        var tree = Koh.Assembler.Syntax.SyntaxTree.Parse(
            Koh.Common.SourceText.From(
                "SECTION \"test\", ROM0\nincbin \"incbin-mg-noexist.bin\"",
                "main.asm"
            )
        );
        var binder = new Koh.Assembler.Binding.Binder(fileResolver: vfs);
