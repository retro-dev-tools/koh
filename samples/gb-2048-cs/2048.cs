...
//
// This single file compiles two ways:
//   * The Koh compiler platform lowers it to a bootable Game Boy ROM
//         Koh C# frontend  ->  typed SSA IR  ->  hand-written SM83 backend  ->  .gb
//   * The plain .NET SDK compiles it as an ordinary unsafe C# program that runs on the desktop
//     against the Koh.GameBoy reference runtime (`dotnet run`).
//
// The trick is the `#if !KOH_FRONTEND` regions below: the Koh frontend parses with KOH_FRONTEND
// defined, so it never sees the `static class` wrapper or the `using` - it just sees the bare
// top-level functions it expects. A stock C# compiler sees a normal unsafe static class. The game
// logic in between is byte-for-byte the same source for both.
//
// It sticks to the supported "Koh C#" systems subset: byte / sbyte / ushort / bool, raw pointers
// (byte*), stackalloc buffers, and the built-in Hardware / Gb surfaces for memory-mapped I/O and the
// video regions. There is no `int`, no heap, no garbage collector - the whole game runs out of a
// single 16-byte board that lives in the Main frame.
//
// Tiles are stored as *exponents*: 0 = empty, 1 = "2", 2 = "4", ... 11 = "2048". Merging two equal
// tiles is therefore just `exponent + 1`, which keeps every value in a byte and makes the
// slide/merge logic trivial to follow.
//
// Controls:  D-pad = slide the board.
//
// Graphics are intentionally minimal: each tile value is drawn as a solid framed block in one of the
// four DMG shades (procedurally generated at boot, since ROM data arrays are not yet in the subset).
...
static unsafe class Game
{
    ...
    {
        byte changed = 0;
        byte* line = stackalloc byte[4]; // one scratch line, refilled for each of the four lines below
        for (byte k = 0; k < 4; k++)
        {
            for (byte j = 0; j < 4; j++)
    ...
    // Build 12 background tiles at the tile-data region: tile 0 is empty (lightest
    // shade); tiles 1..11 are framed blocks whose interior shade cycles so adjacent
    // values read differently. Each tile is 8 rows x 2 bit-planes = 16 bytes.
    ...
    // Paint the board into the background tilemap. Each cell is drawn as a 2x2 block
    // of its value's tile, spaced out into a grid.
    ...
    static void Main()
    {
        ...
        byte* board = stackalloc byte[16];
        for (byte i = 0; i < 16; i++)
            *(board + i) = 0;

        SpawnTile(board, Hardware.DIV);
        SpawnTile(board, Hardware.DIV);
        Render(board);
        ...
        while (true)
        {
            ...
            byte moved = 0;
            if ((pressed & 0x02) != 0)
                moved = MoveDir(board, 0); // Left
            else if ((pressed & 0x01) != 0)
                moved = MoveDir(board, 1); // Right
            else if ((pressed & 0x04) != 0)
                moved = MoveDir(board, 2); // Up
            else if ((pressed & 0x08) != 0)
                moved = MoveDir(board, 3); // Down
