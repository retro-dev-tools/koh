...
public readonly record struct CounterModel(
    int Count,
    int Step,
    bool AllowNegative,
    bool WindowOpen,
...
{
    public static CounterModel Update(CounterMsg msg, CounterModel m) =>
        msg switch
        {
            ...
            SetAllowNegative a => m with
            {
                AllowNegative = a.Allow,
                Count = Clamp(m.Count, a.Allow),
            },
    ...
    public static IView<CounterMsg> View(CounterModel m)
    {
        ...
        var menu = new MenuBar<CounterMsg>(
        ...
        var body = new Panel<CounterMsg, ForEach<CounterMsg>>(
            PanelBevel.Raised,
        ...
        var status = new StatusBar<CounterMsg>(
            ImmutableArray.Create(greeting, $"Value: {m.Count}", $"Step: {m.Step}")
        );
