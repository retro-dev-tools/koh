...
- **`MirDecoder.cs`** — lifts a byte region into `MirInstruction`s, computing each
  ...
  arithmetic; the irregular rows are handled per family; illegal/unmodeled opcodes lift
  to `MirEffects.Opaque` (reads/writes everything) so decoding is **total** and any
  consumer treats them as a barrier.

Tested in `tests/Koh.Compiler.Tests/Backends/MirDecoderTests.cs` (13 cases): lossless
...
control-flow classification, MMIO forms, and totality over all 256 + 256 opcodes.

## Migration path (follow-up PRs)

1. **Peephole onto MIR.** Replace `Sm83Peephole`'s private length table and hand-rolled
   flag-liveness scan with `MirDecoder.Decode` + `MirEffects`. Flag-dead detection becomes
