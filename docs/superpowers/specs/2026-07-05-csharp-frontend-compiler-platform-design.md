...
3. **C# frontend MVP.** ✅ *Landed (first slice).* `Frontends/CSharp` uses Roslyn to parse; walks the syntax tree (no semantic model) rejecting out-of-subset constructs and lowering to IR. Supports `static` methods and top-level functions, `byte`/`sbyte`/`ushort`/`short`/`bool` with C-like (non-promoting) typing, locals + parameters as `alloca`s (so control flow needs no phi construction), arithmetic/bitwise/shift/compare, casts, unary `-`/`!`, `=` and compound assignment, `if`/`else`, `while`, method calls, and `return`. Signedness selects signed vs. unsigned div/rem/shift/compare. Verified end-to-end on the emulator: `Add`, a `Sum` while-loop (→45), `Factorial` (mul + `*=`, →120), `Gcd` (`%` loop, →12), `Max` (`if`/`else`), cross-function calls, signed negate, and signed divide. Remaining: `for`, `&&`/`||` (short-circuit), arrays/structs/pointers, hardware-intrinsic surface, and `extern` asm interop.
