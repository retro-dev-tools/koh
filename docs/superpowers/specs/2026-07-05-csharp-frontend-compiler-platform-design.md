...
2. **SM83 backend MVP.** 🚧 *In progress.* First increment landed: single-block `i8` functions (constant/binary arithmetic + bitwise + `ret`) → naive static allocation (one WRAM byte per SSA value, everything through `A`/`B`) → `EmitModel` → **linked to a ROM and executed in the emulator**, validated by both golden-byte and run-and-check-the-accumulator tests. Remaining increments: wider-integer legalization, control flow (branches/phi), `load`/`store`/`gep`/`alloca`, calls + far-call trampolines, and routing instruction selection through `Sm83InstructionTable`.
