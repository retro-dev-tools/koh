...
internal static class Sm83Ops
{
    ...
    public static (IrCompareOp Pred, bool Swap, bool Signed) Normalize(IrCompareOp op) =>
        op switch
        {
    ...
    public static byte AluImmOpcode(IrBinaryOp op, int k) =>
        op switch
        {
    ...
    public static byte AluRegOpcode(IrBinaryOp op, int k) =>
        op switch
        {
