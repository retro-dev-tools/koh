...
internal sealed class SectionPCTracker
{
    ...
    private readonly Dictionary<string, int> _sectionBasePCs = new(
        StringComparer.OrdinalIgnoreCase
    );
    private readonly Dictionary<string, int> _sectionAlignBits = new(
        StringComparer.OrdinalIgnoreCase
    );
    private readonly Dictionary<string, SectionType> _sectionTypes = new(
        StringComparer.OrdinalIgnoreCase
    );
    ...
    public int CurrentPC =>
        _activeSection != null && _sectionPCs.TryGetValue(_activeSection, out var pc) ? pc : 0;
    ...
    public int ActiveAlignBits =>
        _activeSection != null && _sectionAlignBits.TryGetValue(_activeSection, out var bits)
            ? bits
            : 0;
    ...
    public SectionType? ActiveSectionType =>
        _activeSection != null && _sectionTypes.TryGetValue(_activeSection, out var t) ? t : null;

    /// <summary>Current byte offset from section start (PC minus base PC).</summary>
    public int ActiveSectionOffset =>
        _activeSection != null && _sectionBasePCs.TryGetValue(_activeSection, out var basePC)
            ? CurrentPC - basePC
            : CurrentPC;

    /// <summary>Returns the maximum byte capacity for the given section type.</summary>
    public static int MaxSizeForType(SectionType type) =>
        type switch
        {
    ...
    public void BeginLoad(string loadSectionName, int basePC)
    {
        if (_loadSection != null)
            return; // nested LOAD not supported
