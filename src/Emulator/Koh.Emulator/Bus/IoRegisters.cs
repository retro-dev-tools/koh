using Koh.Emulator.Apu;
using Koh.Emulator.Cgb;
using Koh.Emulator.Cpu;
using Koh.Emulator.Dma;
using Koh.Emulator.Joypad;
using Koh.Emulator.Ppu;
using Koh.Emulator.Serial;
using Koh.Emulator.State;

namespace Koh.Emulator.Bus;
...
public sealed class IoRegisters
{
        ...
        switch (address)
        {
            ...
            case 0xFF00:
            {
                ...
                if (_readJoypad is not null)
                {
                    ...
                    {
                        if (j.IsPressed(JoypadButton.Right))
                            pressed |= 0x01;
                        if (j.IsPressed(JoypadButton.Left))
                            pressed |= 0x02;
                        if (j.IsPressed(JoypadButton.Up))
                            pressed |= 0x04;
                        if (j.IsPressed(JoypadButton.Down))
                            pressed |= 0x08;
                    }
                    if ((selectMask & 0x20) == 0)
                    {
                        if (j.IsPressed(JoypadButton.A))
                            pressed |= 0x01;
                        if (j.IsPressed(JoypadButton.B))
                            pressed |= 0x02;
                        if (j.IsPressed(JoypadButton.Select))
                            pressed |= 0x04;
                        if (j.IsPressed(JoypadButton.Start))
                            pressed |= 0x08;
                    }
                }
            ...
            case 0xFF06:
                return Timer.TMA;
            case 0xFF07:
                return (byte)(Timer.TAC | 0xF8);
            case 0xFF0F:
                return (byte)(_interrupts.IF | 0xE0);

            // PPU registers
            case 0xFF40:
                return _ppu?.LCDC ?? _io[idx];
            case 0xFF41:
                return _ppu is null ? _io[idx] : _ppu.Stat.Read(_ppu.Mode, _ppu.LY == _ppu.LYC);
            case 0xFF42:
                return _ppu?.SCY ?? _io[idx];
            case 0xFF43:
                return _ppu?.SCX ?? _io[idx];
            case 0xFF44:
                return _ppu?.LY ?? _io[idx];
            case 0xFF45:
                return _ppu?.LYC ?? _io[idx];
            case 0xFF47:
                return _ppu?.BGP ?? _io[idx];
            case 0xFF48:
                return _ppu?.OBP0 ?? _io[idx];
            case 0xFF49:
                return _ppu?.OBP1 ?? _io[idx];
            case 0xFF4A:
                return _ppu?.WY ?? _io[idx];
            case 0xFF4B:
                return _ppu?.WX ?? _io[idx];

            // CGB-only registers read $FF on DMG.
            case 0xFF4D:
                return IsCgb ? (_keyOne?.Read() ?? 0xFF) : (byte)0xFF;
            case 0xFF4F:
                return IsCgb ? (_banking?.ReadVbkRegister() ?? 0xFF) : (byte)0xFF;
            case 0xFF70:
                return IsCgb ? (_banking?.ReadSvbkRegister() ?? 0xFF) : (byte)0xFF;

            case 0xFF51:
                return IsCgb ? (_hdma?.Source1 ?? 0xFF) : (byte)0xFF;
            case 0xFF52:
                return IsCgb ? (_hdma?.Source2 ?? 0xFF) : (byte)0xFF;
            case 0xFF53:
                return IsCgb ? (_hdma?.Dest1 ?? 0xFF) : (byte)0xFF;
            case 0xFF54:
                return IsCgb ? (_hdma?.Dest2 ?? 0xFF) : (byte)0xFF;
            case 0xFF55:
            ...
            case 0xFF69:
                return IsCgb ? (_ppu?.BgPalette.ReadData() ?? 0xFF) : (byte)0xFF;
            case 0xFF6A:
                return IsCgb && _ppu is not null ? _ppu.ObjPalette.IndexRegister : (byte)0xFF;
            case 0xFF6B:
                return IsCgb ? (_ppu?.ObjPalette.ReadData() ?? 0xFF) : (byte)0xFF;
            case 0xFF6C:
                return IsCgb && _ppu is not null ? (byte)(_ppu.OPRI | 0xFE) : (byte)0xFF;
            ...
            case 0xFF10:
                return (byte)(_io[idx] | 0x80); // NR10: bit 7 = 1
            case 0xFF11:
                return (byte)(_io[idx] | 0x3F); // NR11: bits 0-5 = 1 (only bits 6-7 readable)
            case 0xFF13:
                return 0xFF; // NR13: write-only
            case 0xFF14:
                return (byte)(_io[idx] | 0xBF); // NR14: bits 0-5, bit 7 = 1 (only bit 6 readable)
            case 0xFF15:
                return 0xFF; // unused
            case 0xFF16:
                return (byte)(_io[idx] | 0x3F); // NR21
            case 0xFF18:
                return 0xFF; // NR23: write-only
            case 0xFF19:
                return (byte)(_io[idx] | 0xBF); // NR24
            case 0xFF1A:
                return (byte)(_io[idx] | 0x7F); // NR30: bits 0-6 = 1
            case 0xFF1B:
                return 0xFF; // NR31: write-only
            case 0xFF1C:
                return (byte)(_io[idx] | 0x9F); // NR32: bits 0-4, bit 7 = 1
            case 0xFF1D:
                return 0xFF; // NR33: write-only
            case 0xFF1E:
                return (byte)(_io[idx] | 0xBF); // NR34
            case 0xFF1F:
                return 0xFF; // unused
            case 0xFF20:
                return 0xFF; // NR41: write-only
            case 0xFF23:
                return (byte)(_io[idx] | 0xBF); // NR44
            case 0xFF26:
                return (byte)(_io[idx] | 0x70); // NR52: bits 4-6 = 1
            ...
            default:
                ...
                if (IsUnmappedIoPort(address))
                    return 0xFF;
                return _io[idx];
        }
    }

    private static bool IsUnmappedIoPort(ushort address) =>
        address switch
        {
            0xFF03 => true,
    ...
    public void Write(ushort address, byte value)
    {
        ...
        switch (address)
        {
            ...
            case 0xFF07:
                ...
                break;
            case 0xFF0F:
                _interrupts.IF = (byte)(value & 0x1F);
                break;
            ...
            case 0xFF4F:
                if (IsCgb)
                    _banking?.WriteVbkRegister(value);
                break;
            case 0xFF70:
                if (IsCgb)
                    _banking?.WriteSvbkRegister(value);
                break;

            // HDMA (inert on DMG)
            case 0xFF51:
                if (IsCgb && _hdma is not null)
                    _hdma.Source1 = value;
                break;
            case 0xFF52:
                if (IsCgb && _hdma is not null)
            ...
            case 0xFF55:
                if (IsCgb)
                    _hdma?.WriteLengthRegister(value);
                break;

            // CGB palette (inert on DMG)
            case 0xFF68:
                if (IsCgb && _ppu is not null)
                    _ppu.BgPalette.IndexRegister = value;
                break;
            case 0xFF69:
                if (IsCgb)
                    _ppu?.BgPalette.WriteData(value);
                break;
            case 0xFF6A:
                if (IsCgb && _ppu is not null)
                    _ppu.ObjPalette.IndexRegister = value;
                break;
            case 0xFF6B:
                if (IsCgb)
                    _ppu?.ObjPalette.WriteData(value);
                break;
            case 0xFF6C:
                if (IsCgb && _ppu is not null)
    ...
    {
        w.WriteBytes(_io);
        w.WriteByte(_interrupts.IF);
        w.WriteByte(_interrupts.IE);
    }
