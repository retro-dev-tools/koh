...
internal static class Mbc3
{
    ...
    {
        if (!cart.Mbc1_RamEnabled)
            return 0xFF;
        byte sel = cart.Mbc1_BankHigh;
        if (sel < 0x04)
        {
            if (cart.Ram.Length == 0)
                return 0xFF;
    ...
    public static void WriteRom(Cartridge cart, ushort address, byte value)
    {
        ...
        if (address < 0x4000)
        {
            cart.Mbc1_BankLow = (byte)(value & 0x7F);
            return;
        }
        if (address < 0x6000)
        {
            cart.Mbc1_BankHigh = value;
            return;
        }
    ...
    public static void WriteRam(Cartridge cart, ushort address, byte value)
    {
        ...
        {
            case 0x08:
                cart.Rtc.Seconds = (byte)(value & 0x3F);
                break;
            case 0x09:
                cart.Rtc.Minutes = (byte)(value & 0x3F);
                break;
            case 0x0A:
                cart.Rtc.Hours = (byte)(value & 0x1F);
                break;
            case 0x0B:
                cart.Rtc.DayLow = value;
                break;
            case 0x0C:
                cart.Rtc.DayHighAndFlags = (byte)(value & 0xC1);
                break;
        }
    }
}
