...
public sealed class SetDataBreakpointsHandler
{
    ...
    public SetDataBreakpointsHandler(DebugSession session)
    {
        _session = session;
    }
    ...
    public Response Handle(Request request)
    {
        var args = request.Arguments?.Deserialize(
            DapJsonContext.Default.SetDataBreakpointsArguments
        );
        if (args is null)
        ...
        foreach (var dbp in args.Breakpoints)
        {
            ...
            {
                results.Add(
                    new Breakpoint { Verified = false, Message = $"invalid dataId '{dbp.DataId}'" }
                );
                continue;
            }
            ...
            if (access is "read" or "readWrite")
                _session.Watchpoints.Read[address] = info;
            if (access is "write" or "readWrite")
                _session.Watchpoints.Write[address] = info;
