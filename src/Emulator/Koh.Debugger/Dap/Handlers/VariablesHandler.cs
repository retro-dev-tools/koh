...
public sealed class VariablesHandler
{
    ...
    public VariablesHandler(DebugSession session)
    {
        _session = session;
    }
    ...
    private static Variable[] RegistersScope(Emulator.GameBoySystem gb)
    {
    ...
    private static Variable[] SymbolsScope(DebugSession session)
    {
        ...
        const int Limit = 200;
        return session
            .DebugInfo.SymbolMap.All.Take(Limit)
    ...
    private static Variable[] SourceContextScope(DebugSession session, Emulator.GameBoySystem gb)
    {
        ...
        var nearest = session
            .DebugInfo.SymbolMap.All.Where(s => s.Bank == bank && s.Address <= pc)
    ...
    private static Variable[] HardwareScope(Emulator.GameBoySystem gb)
    {
        static string H8(byte v) => "$" + v.ToString("X2");
        return
        [
