...
public static class Sm83InstructionTable
{
    ...
    private static IEnumerable<InstructionDescriptor> BuildTable()
    {
        ...
        var regs8 = new[]
        {
            (OperandPattern.RegB, 0),
            (OperandPattern.RegC, 1),
            (OperandPattern.RegD, 2),
            (OperandPattern.RegE, 3),
            (OperandPattern.RegH, 4),
            (OperandPattern.RegL, 5),
        ...
        yield return I(
            "LD",
            [OperandPattern.IndHL, OperandPattern.Imm8],
            [0x36],
            2,
            [new(EmitRuleKind.AppendImm8, 1)]
        );
        ...
        yield return I(
            "LD",
            [OperandPattern.RegBC, OperandPattern.Imm16],
            [0x01],
            3,
        ...
        yield return I(
            "LD",
            [OperandPattern.RegDE, OperandPattern.Imm16],
            [0x11],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        yield return I(
            "LD",
            [OperandPattern.RegHL, OperandPattern.Imm16],
            [0x21],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        yield return I(
            "LD",
            [OperandPattern.RegSP, OperandPattern.Imm16],
            [0x31],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        ...
        yield return I(
            "LD",
            [OperandPattern.RegHL, OperandPattern.SpPlusImm8],
            [0xF8],
            2,
            [new(EmitRuleKind.AppendImm8, 1)]
        );
        ...
        yield return I(
            "LD",
            [OperandPattern.RegA, OperandPattern.IndImm16],
            [0xFA],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        yield return I(
            "LD",
            [OperandPattern.IndImm16, OperandPattern.RegA],
            [0xEA],
            3,
        ...
        yield return I(
            "LD",
            [OperandPattern.IndImm16, OperandPattern.RegSP],
            [0x08],
            3,
            [new(EmitRuleKind.AppendImm16LE, 0)]
        );
        ...
        yield return I(
            "LDH",
            [OperandPattern.RegA, OperandPattern.IndImm8],
            [0xF0],
            2,
            [new(EmitRuleKind.AppendImm8, 1)]
        );
        yield return I(
            "LDH",
            [OperandPattern.IndImm8, OperandPattern.RegA],
            [0xE0],
            2,
        ...
        for (int i = 0; i < aluOps.Length; i++)
        {
            ...
            yield return I(
                mnem,
                [OperandPattern.RegA, OperandPattern.IndHL],
                [(byte)(baseOp | 6)],
                1
            );
            ...
            yield return I(
                mnem,
                [OperandPattern.RegA, OperandPattern.Imm8],
                [(byte)immOp],
                2,
                [new(EmitRuleKind.AppendImm8, 1)]
            );
        ...
        yield return I(
            "ADD",
            [OperandPattern.RegSP, OperandPattern.Imm8Signed],
            [0xE8],
            2,
            [new(EmitRuleKind.AppendImm8, 1)]
        );
        ...
        yield return I(
            "JP",
            [OperandPattern.CondNZ, OperandPattern.Imm16],
            [0xC2],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        yield return I(
            "JP",
            [OperandPattern.CondZ, OperandPattern.Imm16],
            [0xCA],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        yield return I(
            "JP",
            [OperandPattern.CondNC, OperandPattern.Imm16],
            [0xD2],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        yield return I(
            "JP",
            [OperandPattern.CondC, OperandPattern.Imm16],
            [0xDA],
            3,
            [new(EmitRuleKind.AppendImm16LE, 1)]
        );
        ...
        yield return I(
            "JR",
            [OperandPattern.CondNZ, OperandPattern.Imm8Signed],
            [0x20],
            2,
        ...
        yield return I(
            "JR",
            [OperandPattern.CondZ, OperandPattern.Imm8Signed],
            [0x28],
            2,
            [new(EmitRuleKind.AppendRelative8, 1)]
        );
        yield return I(
            "JR",
            [OperandPattern.CondNC, OperandPattern.Imm8Signed],
            [0x30],
            2,
            [new(EmitRuleKind.AppendRelative8, 1)]
        );
        yield return I(
            "JR",
            [OperandPattern.CondC, OperandPattern.Imm8Signed],
            [0x38],
            2,
            [new(EmitRuleKind.AppendRelative8, 1)]
        );
        ...
        foreach (var (reg, code) in cbRegs)
            for (int bit = 0; bit < 8; bit++)
            {
                yield return I(
                    mnem,
                    [OperandPattern.Imm3, reg],
                    [0xCB, (byte)(baseOp | (bit << 3) | code)],
                    2,
    ...
    private static InstructionDescriptor I(
        string mnemonic,
        OperandPattern[] operands,
        byte[] encoding,
        int size,
        EmitRule[]? emitRules = null,
        int? expectedBitIndex = null
    )
    {
