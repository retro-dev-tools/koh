...
public sealed class WriteMemoryHandler
{
    ...
    public WriteMemoryHandler(DebugSession session)
    {
        _session = session;
    }
    ...
    public Response Handle(Request request)
    {
        ...
        if (system is null)
            ...
            {
                Success = false,
                Message = "writeMemory: no active debug session",
            };
        ...
        if (!DataBreakpointInfoHandler.TryParseAddress(args.MemoryReference, out ushort baseAddr))
            ...
            {
                Success = false,
                Message = $"writeMemory: invalid memoryReference '{args.MemoryReference}'",
            };
        ...
        catch (FormatException)
        {
            ...
            {
                Success = false,
                Message = "writeMemory: data is not valid base64",
            };
        }
        ...
        for (int i = 0; i < bytes.Length; i++)
        {
            ...
            if (!system.DebugWriteByte(addr, bytes[i]))
                break;
            written++;
        }
        ...
        if (bytes.Length > 0 && written == 0)
        {
            return new Response
            {
                Success = false,
                Message =
                    "writeMemory: write rejected (emulator must be paused; check memory region is writable)",
