...
public sealed partial class CSharpFrontend
{
    internal static CsType ResolveType(TypeSyntax type, IReadOnlyDictionary<string, CsEnum> enums)
    {
        ...
        if (type is PointerTypeSyntax pointer)
            return new CsType(
                IrType.Pointer(ResolveType(pointer.ElementType, enums).Ir),
                Signed: false
            );
    ...
    internal static long ConstEval(
        ...
        expr switch
        {
            ...
            MemberAccessExpressionSyntax
            {
                ...
                when enums is not null
                    && enums.TryGetValue(type.Identifier.Text, out var en)
            ...
            PrefixUnaryExpressionSyntax { RawKind: (int)SyntaxKind.UnaryMinusExpression } u =>
                -ConstEval(u.Operand, lookup, enums, unsigned),
    ...
    {
        long l = ConstEval(bin.Left, lookup, enums, unsigned),
            r = ConstEval(bin.Right, lookup, enums, unsigned);
