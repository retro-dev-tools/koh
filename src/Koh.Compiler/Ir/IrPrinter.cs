...
public static class IrPrinter
{
    ...
    private static void PrintInstruction(StringBuilder sb, IrInstruction instr, Naming names)
    {
        ...
        switch (instr)
        {
            case BinaryInstruction b:
                sb.Append(b.Mnemonic).Append(' ').Append(b.Left.Type).Append(' ')
                  .Append(Op(b.Left, names)).Append(", ").Append(Op(b.Right, names));
                break;
            case CompareInstruction c:
                sb.Append("icmp ").Append(c.Op.ToString().ToLowerInvariant()).Append(' ')
                  .Append(c.Left.Type).Append(' ')
                  .Append(Op(c.Left, names)).Append(", ").Append(Op(c.Right, names));
                break;
            case ConvInstruction cv:
                sb.Append(cv.Mnemonic).Append(' ').Append(cv.Operand.Type).Append(' ')
                  .Append(Op(cv.Operand, names)).Append(" to ").Append(cv.Type);
                break;
            ...
            case StoreInstruction s:
                sb.Append("store ").Append(s.Value.Type).Append(' ').Append(Op(s.Value, names))
                  .Append(", ").Append(s.Pointer.Type).Append(' ').Append(Op(s.Pointer, names));
                break;
            case GetElementPtrInstruction g:
                sb.Append("gep ").Append(g.ElementType).Append(", ")
                  .Append(g.BasePointer.Type).Append(' ').Append(Op(g.BasePointer, names)).Append(", ")
                  .Append(g.Index.Type).Append(' ').Append(Op(g.Index, names));
                break;
            case CallInstruction call:
                sb.Append("call ").Append(call.Callee.ReturnType).Append(" @").Append(call.Callee.Name).Append('(');
                for (int i = 0; i < call.Arguments.Count; i++)
                {
                    if (i > 0) sb.Append(", ");
                    sb.Append(call.Arguments[i].Type).Append(' ').Append(Op(call.Arguments[i], names));
                }
                sb.Append(')');
                break;
            case PhiInstruction phi:
                ...
                for (int i = 0; i < phi.Incomings.Count; i++)
                {
                    ...
                    sb.Append("[ ").Append(Op(val, names)).Append(", ").Append(names.Block(blk)).Append(" ]");
                }
                break;
            ...
            case CondBrInstruction cb:
                sb.Append("condbr ").Append(Op(cb.Condition, names)).Append(", ")
                  .Append(names.Block(cb.IfTrue)).Append(", ").Append(names.Block(cb.IfFalse));
                break;
            case SwitchInstruction sw:
                sb.Append("switch ").Append(sw.Value.Type).Append(' ').Append(Op(sw.Value, names))
                  .Append(", ").Append(names.Block(sw.Default)).Append(" [ ");
                for (int i = 0; i < sw.Cases.Count; i++)
                {
                    if (i > 0) sb.Append(", ");
                    sb.Append(sw.Cases[i].Case.Value).Append(": ").Append(names.Block(sw.Cases[i].Target));
                }
                sb.Append(" ]");
                break;
            default:
    ...
    private static string Op(IrValue v, Naming names) => v switch
    {
    ...
    {
        private readonly Dictionary<IrValue, string> _values = new(ReferenceEqualityComparer.Instance);
        private readonly Dictionary<IrBasicBlock, string> _blocks = new(ReferenceEqualityComparer.Instance);
        ...
        public string Value(IrValue v) =>
            _values.TryGetValue(v, out var n) ? n : "<undef>";

        public string Block(IrBasicBlock b) =>
            _blocks.TryGetValue(b, out var n) ? n : "<unknown>";
    }
}
