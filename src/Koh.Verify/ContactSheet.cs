...
public static class ContactSheet
{
    ...
    public static void WriteStrip(string path, IList<Tile> tiles,
                                   int scale = 2, int padding = 6, int labelHeight = 14,
                                   byte bgR = 24, byte bgG = 24, byte bgB = 28,
                                   byte labelR = 240, byte labelG = 240, byte labelB = 240)
        => Write(path, tiles, columns: tiles.Count, scale: scale, padding: padding,
                 labelHeight: labelHeight, bgR: bgR, bgG: bgG, bgB: bgB,
                 labelR: labelR, labelG: labelG, labelB: labelB);

    public static void Write(string path, IList<Tile> tiles, int columns,
                              int scale = 2, int padding = 12, int labelHeight = 18,
                              byte bgR = 24, byte bgG = 24, byte bgB = 28,
                              byte labelR = 240, byte labelG = 240, byte labelB = 240)
    {
        ...
        for (int idx = 0; idx < tiles.Count; idx++)
        {
            ...
            DrawLabel(canvas, totalW, totalH,
                      col * cellW + padding, row * cellH + 4,
                      cellW - padding * 2, labelHeight, tiles[idx].Label,
                      labelR, labelG, labelB);
        }
        PngEncoder.WriteFromRgba(path, canvas, totalW, totalH, scale: 1);
    }

    private static void BlitScaledRgb(byte[] dst, int dstStride, int x0, int y0,
                                       byte[] src, int srcW, int srcH, int scale)
    {
        ...
        for (int sx = 0; sx < srcW; sx++)
        {
            ...
            for (int dx = 0; dx < scale; dx++)
            {
                ...
                dst[dpos+0] = r; dst[dpos+1] = g; dst[dpos+2] = b; dst[dpos+3] = 0xFF;
            }
        }
    }
    ...
    private static void DrawLabel(byte[] canvas, int w, int h, int x, int y,
                                   int areaW, int areaH, string text,
                                   byte r, byte g, byte b)
    {
        string up = text.ToUpperInvariant();
        const int charW = 6, charH = 8, glyphScale = 2;
            ...
            for (int ry = 0; ry < 7; ry++)
            {
                ...
                for (int rx = 0; rx < 5; rx++)
                {
                    ...
                    for (int dx = 0; dx < glyphScale; dx++)
                    {
                        ...
                        if ((uint)cx >= w || (uint)cy >= h) continue;
                        int p = (cy * w + cx) * 4;
                        canvas[p+0] = r; canvas[p+1] = g; canvas[p+2] = b; canvas[p+3] = 0xFF;
                    }
                }
            }
            sx += charW * glyphScale;
